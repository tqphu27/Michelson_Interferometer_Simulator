...
class ScreenCanvas(MyMplCanvas):
    ...
    def changeM1_dir(self, text):
        y = float(text)
        self.simulation.setMirrorM1_dir([0,y,0])
        self.axes.cla()
        showPattern(self.axes, self.simulation, self.is_colored)
        self.draw()

    def changeM1(self, text):
        nx = float(text) # small angle approx.
        self.simulation.setMirrorM1([nx,-1,0])
        self.axes.cla()
        showPattern(self.axes, self.simulation, self.is_colored)
        self.draw()

    def changeM2_dir(self, text):
        x = float(text)
        self.simulation.setMirrorM2_dir([x,0,0])
        self.axes.cla()
        showPattern(self.axes, self.simulation, self.is_colored)
        self.draw()
...
class InputdialogDemo(QWidget):
    def __init__(self,parent=None):
        ...
        self.btn_M1_dir=QPushButton("M1 location:") 
        self.btn_M1_dir.clicked.connect(self.getM1_dir)
        self.le_M1_dir=QLineEdit()
        self.le_M1_dir.setText('100')
        layout.addRow(self.btn_M1_dir,self.le_M1_dir)
        ...
        self.btn_M2_dir.clicked.connect(self.getM2_dir)
        self.le_M2_dir=QLineEdit()
        self.le_M2_dir.setText('100')
        layout.addRow(self.btn_M2_dir,self.le_M2_dir)
    ...
    def getM1_dir(self):
        ...
        if ok:
            self.le_M1_dir.setText(str(text))
    
    def getM1(self):
        text, ok = QInputDialog.getText(self,'Text Input Dialog','M1 angle?')
        if ok:
            self.le_M1.setText(str(text))

    def getM2_dir(self):
        text, ok = QInputDialog.getText(self,'Text Input Dialog','M2 location?')
        if ok:
            self.le_M2_dir.setText(str(text))
...
class ApplicationWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        idd.le_M1_dir.textChanged.connect(sc.changeM1_dir)
        idd.le_M1.textChanged.connect(sc.changeM1)
        idd.le_M2_dir.textChanged.connect(sc.changeM2_dir)
